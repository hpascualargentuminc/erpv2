...
class AccountBankStatementImport(models.TransientModel):
    ...
    def _parse_file(self, data_file):
        if len(self.attachment_ids) > 1:
            raise UserError(_('Solo un archivo puede ser seleccionado.'))
        ...
        for account in ofx.accounts:
            ...
            # Dado que el OFX de Banco Popular siempre tiene moneda USD, y eso genera errores, comentamos esta línea.
            ...
            vals_bank_statement.append({
                ...
                # Se elimina el cálculo automático de saldo inicial y final, para que el cliente lo cargue a mano a estos valores.
                #'balance_start': float(account.statement.balance) - total_amt,
                #'balance_end_real': account.statement.balance,
                'name': "BPD "+ str(date.today().strftime("%Y-%m-%d")) + " Bank Statement"
            })
