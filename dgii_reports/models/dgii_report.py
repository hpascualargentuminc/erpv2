...
class DgiiReport(models.Model):
    ...
    # Este método computa si el reporte previo al que estamos generando aún está pendiente de Enviar.
    ...
    def _get_invoices(self, states, types, cancelled=False):
        """
    ...
    def formated_rnc_cedula(self, vat):
        if vat:
            ...
            if len(clean_vat) in [9, 11]:
                id_type = 1 if len(clean_vat) == 9 else 2
                return (
                    (clean_vat, id_type)
                    if not clean_vat.isspace()
                    else False
                )
            else:
                return False
    ...
    def _get_formated_amount(self, amount):
        ...
        # Siempre va positivo el valor en los reportes, aunque las NC pueden ser valores negativos, aquí con el ABS llevamos todo a POSITIVO.
    ...
    def _compute_606_data(self):
        for rec in self:
            ...
            invoice_ids = self._get_invoices(
                ["not_paid", "in_payment", "paid"], ["in_invoice", "in_refund"]
            )
            ...
            for inv in invoice_ids:
                ...
                rnc_ced = (
                    ...
                    # VER SI ESTO REALMENTE FUNCIONA, Y CON ESTO CONSIGO VALIDAR SI ES EXTERIOR
                    # PROBAR UNA FACTURA AL EXTERIOR, QUE EL VAT QUE PONGA NO SE REFORMATEE EN EL ARCHIVO. QUE QUEDE AS-IS
                    # SI ES UNA FACTURA LOCAL, QUE SE FORMATEE EL RNC/CEDULA SEGUN CORRESPONDA
                    if inv.l10n_latam_document_type_id.l10n_do_ncf_type != "exterior"
                ...
                values = {
                    ...
                    if inv.move_type == "in_refund" or inv.l10n_latam_document_type_id.l10n_do_ncf_type == "debit_note"
                    else False,
                    ...
                    "purchase_perceived_itbis": 0,  # Actualmente esto queda en 0 porque la DGII no lo exije, dado que no existe Normativa.
                    "purchase_perceived_isr": 0,  # Actualmente esto queda en 0 porque la DGII no lo exije, dado que no existe Normativa.
    ...
    def _get_sale_payments_forms(self, invoice_id):
        ...
        if invoice_id.move_type == "out_invoice":
            for payment in invoice_id._get_invoice_payment_widget():
                # NOTA:
                # ---------------------
                # _get_invoice_payment_widget devuelve dentro de payment los siguientes valores del pago (se generan en el método _get_reconciled_vals de account_move.py):
                ...
                if payment_id:
                    ...
                    payments_dict["credit"] += abs(
                        refund_id.amount_total_signed # Monto total en la moneda de la compañia
                    )
    ...
    def _process_op_dict(self, args, invoice):
        op_dict = args
        # Ventas
        if invoice.l10n_latam_document_type_id.l10n_do_ncf_type not in ["debit_note", "credit_note"]:
        ...
        if invoice.l10n_latam_document_type_id.l10n_do_ncf_type in ["credit_note"]:
            op_dict["credit_note"]["qty"] += 1
            op_dict["credit_note"]["amount"] += invoice.amount_untaxed_signed
        # Notas de Débito
        if invoice.l10n_latam_document_type_id.l10n_do_ncf_type in ["debit_note"]:
    ...
    def _compute_607_data(self):
        for rec in self:
            ...
            invoice_ids = self._get_invoices(
                ["not_paid", "in_payment", "paid"], ["out_invoice", "out_refund"]
            )
            ...
            for inv in invoice_ids:
                ...
                for k in payment_dict:
                    payment_dict[k] += (
                        payments[k] * -1 if inv.move_type == "out_refund" else payments[k]
                    )
    ...
    def _compute_608_data(self):
        for rec in self:
            ...
            invoice_ids = self._get_invoices(
                ["cancel"], ["out_invoice", "in_invoice", "out_refund"], ["cancel"]
            ).filtered(
                lambda inv: (
                    (
