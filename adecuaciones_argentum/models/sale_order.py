    ...
    def _update_data_on_opportunity(self):
        for order in self:
            if order.opportunity_id and order.payment_term_id:
                # first_invoice_date = order.payment_term_id.compute(value=order.base_amount_untaxed, date_ref=order.opportunity_id.date_deadline)[0]
                first_invoice_date = order.opportunity_id.date_deadline
